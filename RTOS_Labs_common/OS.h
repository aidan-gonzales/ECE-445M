...
typedef struct tcb {
  ...
  struct tcb *next; // next TCB pointer (in the main TCB linked list)
  uint8_t id; // unique ID for this thread
  uint8_t sleep_state; // true if sleeping
  //uint32_t sleep_until; // if sleeping, stay asleep until TimeMs reaches this value
  uint8_t blocked_state; // true if blocked
...
extern int32_t stacks[MAX_THREADS][STACK_SIZE];
...
typedef struct periodic_thread {
  ...
  uint32_t delta; // holds the amount of ms left before the thread runs
} periodic_thread_t;
