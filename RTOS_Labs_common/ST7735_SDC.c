...
void ST7735_Message(uint32_t d, uint32_t l, char *pt, uint32_t value){
  ...
  // draws the string
  while(*pt) {
    ST7735_DrawChar(x*6, l*10 + d*80, *pt, ST7735_WHITE, ST7735_BLACK, 1);
    pt++;
    x++;
  }
/*
  ...
  */
  ...
  //flips the char array so that it's in the correct order
  for (int j = 0; j < i / 2; j++) {
    char temp = pt2[j];
    pt2[j] = pt2[i - j - 1];
    pt2[i - j - 1] = temp;
  ...
  // prints the newly created string
  while (pt2[k]) {
    ST7735_DrawChar(x*6, l*10 + d*80, pt2[k], ST7735_WHITE, ST7735_BLACK, 1);
    k++;
    x++;
  }
